...
| Wallet request and submission | `BlockheadWalletRequest`, `BlockheadWalletRequestCall`, `BlockheadWalletRequest_Timestamp` | `Local_Internal` owns requested prompt artifacts and ordered call children; wallet sources own rejection, signature, transaction hash, bundle id, and wallet status observations | `wallet-request-not-finality`, `wallet-rejection-and-success` | Wallet success, signature, or transaction hash is not public chain finality. Raw call JSON stays retained evidence referenced by hashes. |
...
| Wallet request/submission artifact vs wallet lifecycle observation | `BlockheadWalletRequest` owns requested prompt and optional submitted time; `BlockheadWalletRequestCall` owns ordered per-call structure (`$walletRequest` + `callIndex`) with hashes/structured fields only; `BlockheadWalletRequest_Timestamp` owns wallet-reported status, signatures, tx hashes, bundle status, receipts, and errors | Wallet status/finality on `BlockheadSessionAction` or wallet request parent, treating signature/tx hash as public receipt; raw call JSON as primary schema fields | `wallet-request-not-finality`, `wallet-rejection-and-success` |
...
- `BlockheadWalletRequest.$$calls`: ordered by `callIndex`; count is retained requested call-child count for batch/inspectable requests, not wallet-reported receipt coverage or public trace coverage.
...
```text
  ...
  Entity BlockheadWalletRequest :: id ; id! p:str, $sessionAction? $:BlockheadSessionAction, $intentOrder? $:BlockheadIntentOrder, $walletConnection? $:BlockheadWalletConnection, walletProtocol? p:enum, caip10? p:{namespace:str,reference:str,accountAddress:str}, requestKind! p:enum, requestMethod! p:str, chainId? p:num, fromAddress? p:evmAddress, toAddress? p:evmAddress, value? p:bigint, callCount? p:num, atomicRequired? p:bool, requestPayloadHash? p:hex, walletCallBundleId? p:str, requestedAt! p:num, submittedAt? p:num, $$calls* $:BlockheadWalletRequestCall, $$timestamps* $:BlockheadWalletRequest_Timestamp
    Sources ::
      ...
      - Planned source binding Wallet_Eip5792
      - Planned source binding Wallet_ProtocolApi
    View :: {"closed":["request id","request kind","latest status"],"content":{"dl":[["request id","wallet protocol","request kind","method","latest status","wallet connection","account","chain id","from/to","value","call count","atomic required","requested time","submitted time"],["latest signature hash","latest transaction hash","latest transaction id","wallet call bundle id","request payload hash"]]},"details":{"tabs":[["Calls",["BlockheadWalletRequestCall list when batch/inspectable calls are retained"]],["Status history",["BlockheadWalletRequest_Timestamp list"]],["Session action",["BlockheadSessionActionView when linked"]],["Intent order",["BlockheadIntentOrderView when linked"]],["Wallet",["BlockheadWalletConnectionView when linked"]],["Chain evidence",["EvmTransaction or protocol transaction rows when the hash/id resolves publicly"]],["Request evidence",["request payload retained locally only when needed"]]]}}
    Notes :: Local wallet request/submission artifact for signing, sending a transaction, or submitting a wallet call batch. It records what the connected wallet was asked to do; wallet-reported status, rejection, signatures, and transaction ids belong on timestamp observations. `requestedAt` is when Blockhead asked the wallet; `submittedAt` is only set when the wallet or provider accepted, signed, sent, or submitted a batch. Rejected prompts can have no submittedAt. `walletProtocol` carries the wallet API family from the wallet catalog; `chainId`, `fromAddress`, `toAddress`, `value`, `callCount`, and `atomicRequired` are EVM-shaped shortcuts used only when the request is EVM-shaped. For EIP-5792, `atomicRequired` mirrors the app's requested batch atomicity; the wallet-reported execution atomicity belongs on BlockheadWalletRequest_Timestamp. Ordered inspectable call structure belongs on `BlockheadWalletRequestCall` children via `$$calls`; raw call JSON stays retained evidence referenced by `requestPayloadHash` / `inputDataHash`, not primary schema fields. Non-EVM request bodies stay behind `requestPayloadHash`, `statusPayloadHash`, and protocol transaction ids until a concrete product flow needs stable child rows for inspected payload parts. It is not a provider intent order, not proof of on-chain inclusion, not wallet account identity, and not final execution truth. Link to public transaction rows only after chain/indexer sources resolve the hash or id.

  Entity BlockheadWalletRequestCall :: $walletRequest+callIndex ; $walletRequest! $:BlockheadWalletRequest, callIndex! p:num, caip2? p:{namespace:str,reference:str}, toAddress? p:evmAddress, value? p:bigint, inputDataHash? p:hex
    Sources ::
      - SourceBinding.Local_Internal
      - Planned source binding Wallet_Eip1193
      - Planned source binding Wallet_Eip5792
    View :: {"closed":["wallet request","call index","to address"],"content":{"dl":[["wallet request","call index","caip2","to address","value","input data hash"]]},"details":{"tabs":[["Wallet request",["BlockheadWalletRequestView"]],["Request evidence",["input/call payload retained locally only when needed"]]]}}
    Notes :: Ordered child of a wallet request for inspectable batch/call structure. Selector is `$walletRequest` + `callIndex`. Store structured request fields and `inputDataHash` only; raw call JSON remains retained evidence, not a primary schema field. Call children do not carry wallet status, signatures, transaction hashes, bundle status, or errors — those stay on `BlockheadWalletRequest_Timestamp`. Wallet success still does not imply public chain finality.
