...
- Bulk edit safety: treat source-of-truth files, generators, generated artifacts, and high-traffic shared files as high-blast-radius surfaces. Do not use broad regex/string rewrites on them. Use parser-backed transforms, key-addressed transforms, or direct targeted patches. Before touching more than one repeated site, inspect and patch one instance, run the smallest relevant parse/generation check, inspect the actual diff, then scale gradually. If the first attempt produces syntax errors or structural drift, stop bulk work immediately and repair from exact local diffs or ask for direction; do not stack more transforms on top of a corrupted file.
- Shell-scripted edits: avoid inline `node -e` / shell one-liners for repository rewrites when the script contains template literals, `$`, backticks, glob-sensitive text, or replacement strings. Put the script in a temporary `.mjs` file or use `apply_patch` so shell expansion cannot alter the edit program.
- Generator/canonical pipeline changes: after each source-of-truth or generator change, verify both the edited source and the final generated product by reading representative final files. Do not rely on format/lint checks alone for visual indentation, section placement, or generated structure.
