...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.A2aTask_Timestamp>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(a2aTaskTimestamps)}
			{@const uniqueA2aTaskTimestamps = [...new Map(a2aTaskTimestamps.values.map((a2aTaskTimestamp) => [a2aTaskTimestamp[EntityMetaKey.SelectorKey], a2aTaskTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.A2aTask_Timestamp}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={a2aTaskTimestamps.totalCount}
				getKey={(a2aTaskTimestamp) => a2aTaskTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueA2aTaskTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
