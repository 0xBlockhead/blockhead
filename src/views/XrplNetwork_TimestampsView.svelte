...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.XrplNetwork_Timestamp>
			title?: string
	...
	>
		...
		{#snippet children(xrplNetworkTimestamps)}
			...
			>
				...
				{#snippet Item({ item: xrplNetworkTimestamp })}
					{@const xrplNetworkTimestampFields = { ...xrplNetworkTimestamp[EntityMetaKey.Selector], ...xrplNetworkTimestamp }}
					{@const selection = select(EntityType.XrplNetwork_Timestamp, xrplNetworkTimestamp[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
