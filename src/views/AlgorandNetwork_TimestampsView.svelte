...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.AlgorandNetwork_Timestamp>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(algorandNetworkTimestamps)}
			{@const uniqueAlgorandNetworkTimestamps = [...new Map(algorandNetworkTimestamps.values.map((algorandNetworkTimestamp) => [algorandNetworkTimestamp[EntityMetaKey.SelectorKey], algorandNetworkTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.AlgorandNetwork_Timestamp}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={algorandNetworkTimestamps.totalCount}
				getKey={(algorandNetworkTimestamp) => algorandNetworkTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueAlgorandNetworkTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
