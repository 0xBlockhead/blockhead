...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.TezosBigMap_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
...
>
	{#snippet Title()}
		<ResourceBoundary resource={tezosBigMapTimestamp}>
			{#snippet Pending()}
				{title || 'tezos big map timestamp'}
			{/snippet}
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			...
			>
				...
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{@const keyCount = resolvedEntity.keyCount}
					{#if keyCount !== undefined && keyCount !== null}
						<div>
							<dt>key count</dt>
							<dd>
