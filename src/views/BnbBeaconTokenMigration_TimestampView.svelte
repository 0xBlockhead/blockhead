...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.BnbBeaconTokenMigration_Timestamp>
			prefetched?: RegisteredEntityProxyPrefetchedData<EntityType.BnbBeaconTokenMigration_Timestamp>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const bnbBeaconTokenMigrationTimestamp = $derived(selection(prefetched[EntityMetaKey.Selector] != null && layout !== EntityLayout.SummaryDetails ? {
	...
	const titleFallback = $derived([String((pendingEntity.timestampMs) ?? '')].filter(Boolean).join(' ') || 'bnb beacon token migration timestamp')
	const viewDomId = $derived('bnb-beacon-token-migration-timestamp-' + encodeURIComponent(stringify(selection.entitySelector ?? prefetched[EntityMetaKey.Selector])))
