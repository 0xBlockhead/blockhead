...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.BitTorrentTrackerScrape_Timestamp>
			prefetched?: RegisteredEntityProxyPrefetchedData<EntityType.BitTorrentTrackerScrape_Timestamp>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const bitTorrentTrackerScrapeTimestamp = $derived(selection(prefetched[EntityMetaKey.Selector] != null && layout !== EntityLayout.SummaryDetails ? {
	...
	const viewDomId = $derived('bit-torrent-tracker-scrape-timestamp-' + encodeURIComponent(stringify(selection.entitySelector ?? prefetched[EntityMetaKey.Selector])))
	...
	{#snippet Content({ open: contentOpen })}
		...
		<dl data-column-item="center">
			...
			>
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{@const complete = resolvedEntity.complete}
			...
			>
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{@const incomplete = resolvedEntity.incomplete}
