...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.AptosTableItem>
			title?: string
...
>
	...
	{#snippet Item({ item: aptosTableItem })}
		...
		{@const selection = select(EntityType.AptosTableItem, aptosTableItem[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
		<AptosTableItemView
			selection={selection}
