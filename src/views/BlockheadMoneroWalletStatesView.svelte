...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.BlockheadMoneroWalletState>
			title?: string
		...
		{#snippet children(blockheadMoneroWalletStates)}
			...
			>
				...
				{#snippet Item({ item: blockheadMoneroWalletState })}
					...
					{@const selection = select(EntityType.BlockheadMoneroWalletState, blockheadMoneroWalletState[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
