<script lang="ts">
	...
	let {
		...
		{
			entityFieldReference: EntityFieldReference<typeof schema, EntityType.ZeroGStorageNode>
			title?: string
			open?: boolean
			id: string
			href?: string
...
<EntitiesList entityType={EntityType.ZeroGStorageNode} {title} bind:open {id} href={href} {...EntitiesListProps}>
	...
	{#snippet body()}
		{#if open}
			...
			{@const nodes = derive(
				parent,
				(parent): readonly Entity<typeof schema, EntityType.ZeroGStorageNode>[] => (
			...
			<EntitiesList
				...
				id={`${id}-items`}
				href={href}
				getKey={(node) => stringify(node[EntityMetaKey.Selector])}
				open={true}
				resource={nodes}
				{title}
