...
>
	{#snippet Title()}
		<ResourceBoundary resource={polkadotAsset}>
			{#snippet Pending()}
				{[String((selection.entitySelector.assetId ?? prefetched.assetId) ?? '')].filter(Boolean).join(' ') || title || 'Polkadot asset'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={polkadotAsset}>
			{#snippet Pending()}
				{[String((selection.entitySelector.assetKind ?? prefetched.assetKind) ?? '')].filter(Boolean).join(' ') || [String((selection.entitySelector.assetId ?? prefetched.assetId) ?? '')].filter(Boolean).join(' ') || title || 'Polkadot asset'}
			{/snippet}
		...
		<dl data-column-item="center">
			...
			<div>
				...
				<dd>
					...
					>
						{#snippet Pending()}
							{@const assetKind = selection.entitySelector.assetKind ?? prefetched.assetKind}
