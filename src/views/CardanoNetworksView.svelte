...
<script lang="ts">
	...
	let {
		...
		id = 'CardanoNetworks-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.CardanoNetwork>
			title?: string
	...
	>
		...
		{#snippet children(cardanoNetworks)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={cardanoNetworks.totalCount}
				getKey={(cardanoNetwork) => cardanoNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueCardanoNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No Cardano networks yet.</p>
					{/if}
				{/snippet}
