...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BridgeTransfer_Timestamp>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(bridgeTransferTimestamps)}
			{@const uniqueBridgeTransferTimestamps = [...new Map(bridgeTransferTimestamps.values.map((bridgeTransferTimestamp) => [bridgeTransferTimestamp[EntityMetaKey.SelectorKey], bridgeTransferTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.BridgeTransfer_Timestamp}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={bridgeTransferTimestamps.totalCount}
				getKey={(bridgeTransferTimestamp) => bridgeTransferTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueBridgeTransferTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
