...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.AiDocument>
			title?: string
...
>
	...
	{#snippet Item({ item: aiDocument })}
		...
		{@const selection = select(EntityType.AiDocument, aiDocument[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
		<AiDocumentView
			selection={selection}
