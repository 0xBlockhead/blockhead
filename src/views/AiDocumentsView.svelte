...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.AiDocument>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(aiDocuments)}
			{@const uniqueAiDocuments = [...new Map(aiDocuments.values.map((aiDocument) => [aiDocument[EntityMetaKey.SelectorKey], aiDocument])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={aiDocuments.totalCount}
				getKey={(aiDocument) => aiDocument[EntityMetaKey.SelectorKey]}
				items={uniqueAiDocuments}
			>
				{#snippet Empty()}
					{#if emptyText != null}
