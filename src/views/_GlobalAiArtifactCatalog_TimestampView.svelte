...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType._GlobalAiArtifactCatalog_Timestamp>
			prefetched?: RegisteredEntityProxyPrefetchedData<EntityType._GlobalAiArtifactCatalog_Timestamp>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const globalAiArtifactCatalogTimestamp = $derived(selection(prefetched[EntityMetaKey.Selector] != null && layout !== EntityLayout.SummaryDetails ? {
	...
	const viewDomId = $derived('-global-ai-artifact-catalog-timestamp-' + encodeURIComponent(stringify(selection.entitySelector ?? prefetched[EntityMetaKey.Selector])))
