...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.FilecoinDeal_Timestamp>, 'prefetched'> = $props()

	const deal = $derived(selection.entitySelector.$deal)
	...
	{#snippet Value()}
		<ResourceBoundary resource={filecoinDealTimestamp}>
			{#snippet children(entity)}
				{String(entity.verifiedDeal ?? '') || String(selection.entitySelector.timestampMs)}
			{/snippet}
		</ResourceBoundary>
	{/snippet}
