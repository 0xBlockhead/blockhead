...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.A2aMessage>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
...
>
	{#snippet Title()}
		<ResourceBoundary resource={a2aMessage}>
			{#snippet Pending()}
				{[String((pendingEntity.messageId) ?? '')].filter(Boolean).join(' ') || title || 'A2A message'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={a2aMessage}>
			{#snippet Pending()}
				{[String((pendingEntity.role) ?? '')].filter(Boolean).join(' ') || [String((pendingEntity.messageId) ?? '')].filter(Boolean).join(' ') || title || 'A2A message'}
			{/snippet}
