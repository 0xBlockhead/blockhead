...
<script lang="ts">
	...
	const titleFallback = $derived([String((selection.entitySelector.dataRoot ?? prefetched.dataRoot) ?? '')].filter(Boolean).join(' ') || 'zero g data blob')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={zeroGDataBlob}>
			{#snippet Pending()}
				{[String((selection.entitySelector.dataRoot ?? prefetched.dataRoot) ?? '')].filter(Boolean).join(' ') || title || 'zero g data blob'}
			{/snippet}
	...
	{#snippet HeadingAfter()}
		<ResourceBoundary resource={zeroGDataBlob}>
			{#snippet Pending()}
				{@const sizeBytes0 = prefetched.sizeBytes}
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					{@const erasureCodingScheme = prefetched.erasureCodingScheme}
			...
			>
				{#snippet Pending()}
					{@const aggregatedSignature = prefetched.aggregatedSignature}
