...
>
	{#snippet Item({ item: farcasterCastTimestamp })}
		...
		<EntityView
			...
			href={
				(
					'fid' in farcasterCastTimestampSelector.$cast
					&& 'hash' in farcasterCastTimestampSelector.$cast ?
						resolve(
							...
							{
								fid: String(farcasterCastTimestampSelector.$cast.fid),
								hash: String(farcasterCastTimestampSelector.$cast.hash),
		...
		>
			{#snippet Title()}
				{[(farcasterCastTimestamp.$cast.text ?? ''), String(farcasterCastTimestampSelector.$cast.hash)].filter(Boolean).join(' ') || 'Farcaster cast'}
			{/snippet}

			{#snippet Value()}
				{String(farcasterCastTimestampSelector.timestampMs)}
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
