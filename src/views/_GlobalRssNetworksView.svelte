...
<script lang="ts">
	...
	let {
		...
		id = 'GlobalRssNetworks-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType._GlobalRssNetwork>
			title?: string
	...
	>
		...
		{#snippet children(globalRssNetworks)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={globalRssNetworks.totalCount}
				getKey={(globalRssNetwork) => globalRssNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueGlobalRssNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No Global RSS networks yet.</p>
					{/if}
				{/snippet}
