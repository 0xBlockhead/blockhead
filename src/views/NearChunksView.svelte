...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.NearChunk>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(nearChunks)}
			{@const uniqueNearChunks = [...new Map(nearChunks.values.map((nearChunk) => [nearChunk[EntityMetaKey.SelectorKey], nearChunk])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={nearChunks.totalCount}
				getKey={(nearChunk) => nearChunk[EntityMetaKey.SelectorKey]}
				items={uniqueNearChunks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
