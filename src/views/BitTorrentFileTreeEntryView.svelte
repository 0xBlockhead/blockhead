			...
			>
				{#snippet children(bitTorrentFile)}
					{#if bitTorrentFile != null}
						{@const bitTorrentFileInitial = untrack(() => bitTorrentFile)}
						<div>
							<dt>file</dt>
							<dd>
								<BitTorrentFileView
									selection={select(EntityType.BitTorrentFile, (bitTorrentFile ?? bitTorrentFileInitial)[EntityMetaKey.Selector])}
									prefetched={bitTorrentFile ?? bitTorrentFileInitial}
									layout={EntityLayout.Value}
								/>
							</dd>
						</div>
