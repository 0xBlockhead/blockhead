...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.HederaScheduleSignature>
			title?: string
		...
		{#snippet children(hederaScheduleSignatures)}
			...
			>
				...
				{#snippet Item({ item: hederaScheduleSignature })}
					...
					{@const selection = select(EntityType.HederaScheduleSignature, hederaScheduleSignature[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
