...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BlockheadWorkspace>
			title?: string
	...
	>
		...
		{#snippet children(blockheadWorkspaces)}
			...
			>
				...
				{#snippet Item({ item: blockheadWorkspace }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.BlockheadWorkspace> })}
					{@const blockheadWorkspaceFields = { ...blockheadWorkspace[EntityMetaKey.Selector], ...blockheadWorkspace }}
					<BlockheadWorkspaceView
						selection={select(EntityType.BlockheadWorkspace, blockheadWorkspace[EntityMetaKey.Selector], { sources: selection.sources })}
