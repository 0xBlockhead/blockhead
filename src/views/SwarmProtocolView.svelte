...
>
	{#snippet Title()}
		<ResourceBoundary resource={swarmProtocol}>
			{#snippet Pending()}
				{[String((pendingEntity.protocolName) ?? '')].filter(Boolean).join(' ') || title || 'Swarm protocol'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={swarmProtocol}>
			{#snippet Pending()}
				{[String((pendingEntity.relationshipModel) ?? '')].filter(Boolean).join(' ') || [String((pendingEntity.protocolName) ?? '')].filter(Boolean).join(' ') || title || 'Swarm protocol'}
			{/snippet}
