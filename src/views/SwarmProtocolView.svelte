...
<script lang="ts">
	...
	const titleFallback = $derived([String((prefetched.protocolName) ?? '')].filter(Boolean).join(' ') || 'Swarm protocol')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={swarmProtocol}>
			{#snippet Pending()}
				{[String((prefetched.protocolName) ?? '')].filter(Boolean).join(' ') || title || 'Swarm protocol'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={swarmProtocol}>
			{#snippet Pending()}
				{[String((prefetched.relationshipModel) ?? '')].filter(Boolean).join(' ') || [String((prefetched.protocolName) ?? '')].filter(Boolean).join(' ') || title || 'Swarm protocol'}
			{/snippet}
