...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.XrplLedgerEntry>
			title?: string
		...
		{#snippet children(xrplLedgerEntries)}
			...
			>
				...
				{#snippet Item({ item: xrplLedgerEntry })}
					...
					{@const selection = select(EntityType.XrplLedgerEntry, xrplLedgerEntry[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
					<XrplLedgerEntryView
						selection={selection}
