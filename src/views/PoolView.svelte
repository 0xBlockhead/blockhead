<script lang="ts">
	...
	const displayTitle = $derived(
		poolField?.token0Symbol != null && poolField?.token1Symbol != null ?
			`${poolField.token0Symbol} / ${poolField.token1Symbol}`
		:	entityId.id,
	)
		...
		{:else}
			...
			>
				...
				>
					...
					{#if rows?.[0]?.row == null}
						<p data-text="muted">
							No liquidity pool row in collections yet (no resolver for this pool).
						</p>
					{:else}
						<dl>
							{#if poolField?.fee != null}
								<div>
									<dt>Fee</dt>
									<dd>{String(poolField.fee)}</dd>
								</div>
							{/if}
							{#if poolField?.tickSpacing != null}
								<div>
									<dt>Tick spacing</dt>
									<dd>{String(poolField.tickSpacing)}</dd>
								</div>
							{/if}
							{#if poolField?.v4PoolId != null}
								<div>
									<dt>v4 pool id</dt>
									<dd>
										<TruncatedValue
