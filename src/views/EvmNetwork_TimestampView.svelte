...
<EntityView
	...
	href={
		href ?? (pendingEntity.$network !== undefined && pendingEntity.$network.slug !== undefined && pendingEntity.timestampMs !== undefined && pendingEntity.source !== undefined ? resolve('/network/[network=networkCaip2OrNetworkSlug]/observations/[timestampMs=nonNegativeInteger]/[source=stringSegment]', {
...
>
	{#snippet Title()}
		<ResourceBoundary resource={evmNetworkTimestamp}>
			{#snippet Pending()}
				{[(String((pendingEntity.blockHeight) ?? '') ? 'Block ' + String((pendingEntity.blockHeight) ?? '') : ''), String((pendingEntity.timestampMs) ?? '')].filter(Boolean).join(' ') || title || 'EVM network timestamp'}
			{/snippet}
