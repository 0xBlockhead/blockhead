...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.BlockheadRadicleSeedObservation_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const blockheadRadicleSeedObservationTimestamp = $derived(selection({
		sources: [
...
>
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			...
			<ResourceBoundary
				resource={selection[EntityProxyField]<EntityType.BlockheadRadicleNodeState, false>('$observerNode')}
			>
		...
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					...
					{#if refCount !== undefined && refCount !== null}
						<div>
							<dt>ref count</dt>
							<dd>
								<NumberValue value={Number(refCount)} />
							</dd>
						</div>
					{/if}
				{/snippet}
				...
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{@const refCount = resolvedEntity.refCount}
					{#if refCount !== undefined && refCount !== null}
						<div>
							<dt>ref count</dt>
							<dd>
