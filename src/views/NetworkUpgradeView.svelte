...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.NetworkUpgrade>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
...
>
	{#snippet Title()}
		<ResourceBoundary resource={networkUpgrade}>
			{#snippet Pending()}
				{[String((pendingEntity.name) ?? ''), String((pendingEntity.upgradeId) ?? '')].filter(Boolean).join(' ') || title || 'network upgrade'}
			{/snippet}
		...
		<dl data-column-item="center">
			...
			<div>
				...
				<dd>
					...
					>
						{#snippet Pending()}
							{@const upgradeId = pendingEntity.upgradeId}
