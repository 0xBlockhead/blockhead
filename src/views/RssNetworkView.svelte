...
<script lang="ts">
	...
	const titleFallback = $derived([String((prefetched.protocolName) ?? '')].filter(Boolean).join(' ') || 'RSS / Atom')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={rssNetwork}>
			{#snippet Pending()}
				{[String((prefetched.protocolName) ?? '')].filter(Boolean).join(' ') || title || 'RSS / Atom'}
			{/snippet}
