...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.AvailDataSubmission>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(availDataSubmissions)}
			{@const uniqueAvailDataSubmissions = [...new Map(availDataSubmissions.values.map((availDataSubmission) => [availDataSubmission[EntityMetaKey.SelectorKey], availDataSubmission])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.AvailDataSubmission}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={availDataSubmissions.totalCount}
				getKey={(availDataSubmission) => availDataSubmission[EntityMetaKey.SelectorKey]}
				items={uniqueAvailDataSubmissions}
			>
				{#snippet Empty()}
					{#if emptyText != null}
