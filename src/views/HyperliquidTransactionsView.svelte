	...
	>
		{#snippet children(hyperliquidTransactions)}
			...
			<EntitiesList
				...
				totalCount={hyperliquidTransactions.totalCount}
				getKey={(hyperliquidTransaction) => hyperliquidTransaction[EntityMetaKey.SelectorKey]}
				items={uniqueHyperliquidTransactions}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: hyperliquidTransaction }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.HyperliquidTransaction> })}
