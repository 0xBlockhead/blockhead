...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.RadicleSignedRef>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(radicleSignedRefs)}
			{@const uniqueRadicleSignedRefs = [...new Map(radicleSignedRefs.values.map((radicleSignedRef) => [radicleSignedRef[EntityMetaKey.SelectorKey], radicleSignedRef])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.RadicleSignedRef}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={radicleSignedRefs.totalCount}
				getKey={(radicleSignedRef) => radicleSignedRef[EntityMetaKey.SelectorKey]}
				items={uniqueRadicleSignedRefs}
			>
				{#snippet Empty()}
					{#if emptyText != null}
