...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.BridgeTransfer>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const bridgeTransfer = $derived(selection({
		sources: [
	...
	const titleFallback = $derived([String((prefetched.transferId) ?? '')].filter(Boolean).join(' ') || 'bridge transfer')
	const viewDomId = $derived('bridge-transfer-' + (titleFallback.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'entity').replace(/^-|-$/g, ''))
...
>
	{#snippet Title()}
		<ResourceBoundary resource={bridgeTransfer}>
			{#snippet Pending()}
				{[String((prefetched.transferId) ?? '')].filter(Boolean).join(' ') || title || 'bridge transfer'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={bridgeTransfer}>
			{#snippet Pending()}
				{[String((selection.entitySelector.source ?? prefetched.source) ?? ''), String((prefetched.railId) ?? '')].filter(Boolean).join(' ') || [String((prefetched.transferId) ?? '')].filter(Boolean).join(' ') || title || 'bridge transfer'}
			{/snippet}
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			<div>
				...
				<dd>
					...
					>
						{#snippet Pending()}
							{@const transferId = prefetched.transferId}
			...
			<ResourceBoundary
				resource={selection[EntityProxyField]<EntityType.EvmTransaction, false>('$sourceTx')}
			>
			...
			<ResourceBoundary
				resource={selection[EntityProxyField]<EntityType.EvmTransaction, false>('$destinationTx')}
			>
				{#snippet children(evmTransaction)}
					{#if evmTransaction != null && evmTransaction[EntityMetaKey.Selector] != null}
						<div>
		...
		<dl data-column-item="center">
			<ResourceBoundary
				resource={selection[EntityProxyField]<EntityType.EvmAccount, false>('$sender')}
			>
			...
			<ResourceBoundary
				resource={selection[EntityProxyField]<EntityType.EvmAccount, false>('$recipient')}
			>
				{#snippet children(evmAccount)}
					{#if evmAccount != null && evmAccount[EntityMetaKey.Selector] != null}
						<div>
				...
				{#snippet Pending()}
					...
					{#if amountIn !== undefined && amountIn !== null}
						<div>
							<dt>amount in</dt>
							<dd>
								<NumberValue value={Number(amountIn)} />
							</dd>
						</div>
					{/if}
				{/snippet}
				...
				{#snippet Pending()}
					...
					{#if amountOut !== undefined && amountOut !== null}
						<div>
							<dt>amount out</dt>
							<dd>
								<NumberValue value={Number(amountOut)} />
							</dd>
						</div>
					{/if}
				{/snippet}
	...
	{#snippet Details({ open: detailsOpen })}
		{#if detailsOpen}
			<BridgeTransfer_TimestampsView
				selection={selection[EntityProxyField]<EntityType.BridgeTransfer_Timestamp>('$$timestamps')}
				title='timestamps'
				emptyText='No bridge transfer observations.'
