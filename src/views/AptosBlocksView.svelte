...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.AptosBlock>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(aptosBlocks)}
			{@const uniqueAptosBlocks = [...new Map(aptosBlocks.values.map((aptosBlock) => [aptosBlock[EntityMetaKey.SelectorKey], aptosBlock])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={aptosBlocks.totalCount}
				getKey={(aptosBlock) => aptosBlock[EntityMetaKey.SelectorKey]}
				items={uniqueAptosBlocks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
