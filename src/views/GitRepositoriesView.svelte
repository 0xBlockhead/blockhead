...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.GitRepository>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(gitRepositories)}
			{@const uniqueGitRepositories = [...new Map(gitRepositories.values.map((gitRepository) => [gitRepository[EntityMetaKey.SelectorKey], gitRepository])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={gitRepositories.totalCount}
				getKey={(gitRepository) => gitRepository[EntityMetaKey.SelectorKey]}
				items={uniqueGitRepositories}
			>
				{#snippet Empty()}
					{#if emptyText != null}
