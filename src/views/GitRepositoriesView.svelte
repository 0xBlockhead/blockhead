...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.GitRepository>
			title?: string
...
>
	...
	{#snippet Item({ item: gitRepository })}
		...
		{@const selection = select(EntityType.GitRepository, gitRepository[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
		<GitRepositoryView
			selection={selection}
