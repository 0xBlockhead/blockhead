	...
	{#snippet body()}
		{#if open}
			...
			{@const smartAccounts = derive(
				network,
				(network): Entity<typeof schema, EntityType.Erc4337SmartAccount>[] => (
					(network.$$erc4337SmartAccounts ?? []).slice(0, 16)
				),
			)}
			<EntitiesList
				collapsible={false}
				...
				getSortValue={(smartAccount) => BigInt(smartAccount[EntityMetaKey.Id].address)}
