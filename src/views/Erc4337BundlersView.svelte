	...
	{#snippet Item({ item: erc4337Bundler })}
		...
		>
			...
			{#snippet HeadingAfter()}
				<span data-text="annotation">{erc4337Bundler.$network.name || (erc4337BundlerSelector.$network.caip2 == null ? '' : `${erc4337BundlerSelector.$network.caip2.namespace}:${erc4337BundlerSelector.$network.caip2.reference}`) || 'Network'}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
