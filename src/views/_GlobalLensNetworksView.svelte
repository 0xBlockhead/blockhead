...
<script lang="ts">
	...
	let {
		...
		id = 'GlobalLensNetworks-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType._GlobalLensNetwork>
			title?: string
	...
	>
		...
		{#snippet children(globalLensNetworks)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={globalLensNetworks.totalCount}
				getKey={(globalLensNetwork) => globalLensNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueGlobalLensNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No Global lens networks yet.</p>
					{/if}
				{/snippet}
