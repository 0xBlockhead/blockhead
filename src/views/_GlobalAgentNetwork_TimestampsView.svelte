...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType._GlobalAgentNetwork_Timestamp>
			title?: string
	...
	>
		...
		{#snippet children(globalAgentNetworkTimestamps)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={globalAgentNetworkTimestamps.totalCount}
				getKey={(globalAgentNetworkTimestamp) => globalAgentNetworkTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueGlobalAgentNetworkTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: globalAgentNetworkTimestamp }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType._GlobalAgentNetwork_Timestamp> })}
					...
					<GlobalAgentNetwork_TimestampView
						selection={select(EntityType._GlobalAgentNetwork_Timestamp, globalAgentNetworkTimestamp[EntityMetaKey.Selector], { sources: selection.sources })}
