...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.IbcClient>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(ibcClients)}
			{@const uniqueIbcClients = [...new Map(ibcClients.values.map((ibcClient) => [ibcClient[EntityMetaKey.SelectorKey], ibcClient])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={ibcClients.totalCount}
				getKey={(ibcClient) => ibcClient[EntityMetaKey.SelectorKey]}
				items={uniqueIbcClients}
			>
				{#snippet Empty()}
					{#if emptyText != null}
