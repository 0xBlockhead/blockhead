...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.McpToolCall>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(mcpToolCalls)}
			{@const uniqueMcpToolCalls = [...new Map(mcpToolCalls.values.map((mcpToolCall) => [mcpToolCall[EntityMetaKey.SelectorKey], mcpToolCall])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.McpToolCall}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={mcpToolCalls.totalCount}
				getKey={(mcpToolCall) => mcpToolCall[EntityMetaKey.SelectorKey]}
				items={uniqueMcpToolCalls}
			>
				{#snippet Empty()}
					{#if emptyText != null}
