	...
	{#snippet Item({ item: erc4337AccountFactory })}
		...
		>
			...
			{#snippet HeadingAfter()}
				<span data-text="annotation">{erc4337AccountFactory.$network.name || `${erc4337AccountFactory.$network.caip2.namespace}:${erc4337AccountFactory.$network.caip2.reference}` || 'Network'}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
