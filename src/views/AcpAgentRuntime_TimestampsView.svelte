...
<script lang="ts">
	...
	let {
		...
		open = $bindable(true),
		...EntitiesListProps
	}: EntityListViewProps<EntityType.AcpAgentRuntime_Timestamp> = $props()
...
>
	{#snippet Item({ item: acpAgentRuntimeTimestamp })}
		{@const acpAgentRuntimeTimestampSelector = acpAgentRuntimeTimestamp[EntityMetaKey.Selector]}
		<EntityView
			entityType={EntityType.AcpAgentRuntime_Timestamp}
			...
			href={
				resolve(
					...
					{
						runtimeId: acpAgentRuntimeTimestampSelector.$runtime.runtimeId,
						timestampMs: String(acpAgentRuntimeTimestampSelector.timestampMs),
						source: acpAgentRuntimeTimestampSelector.source,
					}
				)
			}
		>
