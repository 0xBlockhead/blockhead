...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.SuiObjectVersion>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(suiObjectVersions)}
			{@const uniqueSuiObjectVersions = [...new Map(suiObjectVersions.values.map((suiObjectVersion) => [suiObjectVersion[EntityMetaKey.SelectorKey], suiObjectVersion])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.SuiObjectVersion}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={suiObjectVersions.totalCount}
				getKey={(suiObjectVersion) => suiObjectVersion[EntityMetaKey.SelectorKey]}
				items={uniqueSuiObjectVersions}
			>
				{#snippet Empty()}
					{#if emptyText != null}
