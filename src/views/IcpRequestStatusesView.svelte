...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.IcpRequestStatus>
			title?: string
	...
	>
		...
		{#snippet children(icpRequestStatuses)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={icpRequestStatuses.totalCount}
				getKey={(icpRequestStatus) => icpRequestStatus[EntityMetaKey.SelectorKey]}
				items={uniqueIcpRequestStatuses}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: icpRequestStatus }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.IcpRequestStatus> })}
					{@const icpRequestStatusFields = { ...icpRequestStatus[EntityMetaKey.Selector], ...icpRequestStatus }}
					<IcpRequestStatusView
						selection={select(EntityType.IcpRequestStatus, icpRequestStatus[EntityMetaKey.Selector], { sources: selection.sources })}
