...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesSelection<EntityType.BlockheadWalletRequest>
...
>
	...
	{#snippet Item({ item: blockheadWalletRequest })}
		...
		>
			{#snippet Title()}
				{[String((blockheadWalletRequestFields.requestKind) ?? '')].filter(Boolean).join(' ') || 'blockhead wallet request'}
			{/snippet}

			{#snippet Value()}
				{[String((blockheadWalletRequestFields.requestMethod) ?? '')].filter(Boolean).join(' ')}
			{/snippet}

			{#snippet HeadingAfter()}
				<span data-text="annotation">{[String((blockheadWalletRequestFields.requestedAt) ?? '')].filter(Boolean).join(' ')}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
