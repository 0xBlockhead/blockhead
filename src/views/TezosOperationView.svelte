...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.TezosOperation>, 'prefetched'> = $props()

	const operationGroup = $derived(selection.entitySelector.$operationGroup)
	...
	href={
		href === undefined ?
			resolve(
				...
				{
					network: (
						'caip2' in operationGroup.$network.$network ?
					...
					contentIndex: String(selection.entitySelector.contentIndex),
				}
			)
		:
			href ?? undefined
...
>
	{#snippet Content()}
		<dl data-column-item="center">
			...
			>
				{#snippet children(entity)}
					{@const delegateAddress = entity.delegateAddress}
					{#if delegateAddress != null}
						<div>
							<dt>delegate address</dt>
							<dd>
								<TruncatedValue value={delegateAddress} />
							</dd>
						</div>
					{/if}
				{/snippet}
			...
			>
				{#snippet children(entity)}
					{@const feeMutez = entity.feeMutez}
					{#if feeMutez != null}
						<div>
							<dt>fee mutez</dt>
							<dd>
			...
			>
				{#snippet children(entity)}
					{@const storageLimit = entity.storageLimit}
			...
			>
				{#snippet children(entity)}
					{@const storageSize = entity.storageSize}
					{#if storageSize != null}
						<div>
							<dt>storage size</dt>
							<dd>
			...
			>
				{#snippet children(entity)}
					{@const paidStorageSizeDiff = entity.paidStorageSizeDiff}
					{#if paidStorageSizeDiff != null}
						<div>
