...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesSelection<EntityType.FedimintGateway>
...
>
	...
	{#snippet Item({ item: fedimintGateway })}
		...
		<EntityView
			entityType={EntityType.FedimintGateway}
			entitySelector={fedimintGateway[EntityMetaKey.Selector]}
			layout={EntityLayout.Summary}
			open={false}
			showTypeAnnotation={false}
		>
			{#snippet Title()}
				{[String((fedimintGatewayFields.gatewayId) ?? '')].filter(Boolean).join(' ') || 'Fedimint gateway'}
			{/snippet}

			{#snippet Value()}
				{[String((fedimintGatewayFields.apiUrl) ?? '')].filter(Boolean).join(' ')}
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
