...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.BlockheadRadicleNodeState_Timestamp>
			prefetched?: RegisteredEntityProxyPrefetchedData<EntityType.BlockheadRadicleNodeState_Timestamp>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const blockheadRadicleNodeStateTimestamp = $derived(selection(prefetched[EntityMetaKey.Selector] != null && layout !== EntityLayout.SummaryDetails ? {
	...
	const viewDomId = $derived('blockhead-radicle-node-state-timestamp-' + encodeURIComponent(stringify(selection.entitySelector ?? prefetched[EntityMetaKey.Selector])))
...
>
	{#snippet Title()}
		{#if layout !== EntityLayout.SummaryDetails && Object.hasOwn(prefetched, 'alias') && Object.hasOwn(prefetched, 'nodeVersion')}
	...
	{#snippet Value()}
		{#if layout !== EntityLayout.SummaryDetails && Object.hasOwn(prefetched, 'alias') && Object.hasOwn(prefetched, 'nodeVersion')}
			{[String((pendingEntity.alias) ?? ''), String((pendingEntity.nodeVersion) ?? '')].filter(Boolean).join(' ') || [String((pendingEntity.timestampMs) ?? '')].filter(Boolean).join(' ') || titleFallback}
		{:else}
			<ResourceBoundary resource={blockheadRadicleNodeStateTimestamp}>
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{[String((resolvedEntity.alias) ?? ''), String((resolvedEntity.nodeVersion) ?? '')].filter(Boolean).join(' ') || [String((resolvedEntity.timestampMs) ?? '')].filter(Boolean).join(' ') || titleFallback}
				{/snippet}
			</ResourceBoundary>
		{/if}
	{/snippet}
