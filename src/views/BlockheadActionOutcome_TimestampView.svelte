...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.BlockheadActionOutcome_Timestamp>
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.BlockheadActionOutcome_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const blockheadActionOutcomeTimestamp = $derived(selection({
		sources: [
	...
	const titleFallback = $derived([String((pendingEntity.status) ?? '')].filter(Boolean).join(' ') || 'blockhead action outcome timestamp')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={blockheadActionOutcomeTimestamp}>
			{#snippet Pending()}
				{[String((pendingEntity.status) ?? '')].filter(Boolean).join(' ') || title || 'blockhead action outcome timestamp'}
			{/snippet}
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					{@const finality = pendingEntity.finality}
					{#if finality !== undefined && finality !== null}
						<div>
							<dt>finality</dt>
							<dd>
								{String((finality) ?? '')}
				...
				{#snippet children(entity)}
					{@const resolvedEntity = { ...pendingEntity, ...entity }}
					{@const finality = resolvedEntity.finality}
					{#if finality !== undefined && finality !== null}
						<div>
							<dt>finality</dt>
							<dd>
