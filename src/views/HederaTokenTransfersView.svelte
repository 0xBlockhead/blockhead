...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.HederaTokenTransfer>
			title?: string
	...
	>
		...
		{#snippet children(hederaTokenTransfers)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={hederaTokenTransfers.totalCount}
				getKey={(hederaTokenTransfer) => hederaTokenTransfer[EntityMetaKey.SelectorKey]}
				items={uniqueHederaTokenTransfers}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: hederaTokenTransfer }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.HederaTokenTransfer> })}
					{@const hederaTokenTransferFields = { ...hederaTokenTransfer[EntityMetaKey.Selector], ...hederaTokenTransfer }}
					<HederaTokenTransferView
						selection={select(EntityType.HederaTokenTransfer, hederaTokenTransfer[EntityMetaKey.Selector], { sources: selection.sources })}
