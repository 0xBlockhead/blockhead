...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.StellarNetwork>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(stellarNetworks)}
			{@const uniqueStellarNetworks = [...new Map(stellarNetworks.values.map((stellarNetwork) => [stellarNetwork[EntityMetaKey.SelectorKey], stellarNetwork])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.StellarNetwork}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={stellarNetworks.totalCount}
				getKey={(stellarNetwork) => stellarNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueStellarNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
