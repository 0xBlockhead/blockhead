...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.KaspaTransaction>
			title?: string
	...
	>
		...
		{#snippet children(kaspaTransactions)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={kaspaTransactions.totalCount}
				getKey={(kaspaTransaction) => kaspaTransaction[EntityMetaKey.SelectorKey]}
				items={uniqueKaspaTransactions}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: kaspaTransaction }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.KaspaTransaction> })}
					{@const kaspaTransactionFields = { ...kaspaTransaction[EntityMetaKey.Selector], ...kaspaTransaction }}
					<KaspaTransactionView
						selection={select(EntityType.KaspaTransaction, kaspaTransaction[EntityMetaKey.Selector], { sources: selection.sources })}
