...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BlockheadLocalMediaIngest>
			title?: string
	...
	>
		...
		{#snippet children(blockheadLocalMediaIngests)}
			...
			>
				...
				{#snippet Item({ item: blockheadLocalMediaIngest }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.BlockheadLocalMediaIngest> })}
					...
					<BlockheadLocalMediaIngestView
						selection={select(EntityType.BlockheadLocalMediaIngest, blockheadLocalMediaIngest[EntityMetaKey.Selector], { sources: selection.sources })}
