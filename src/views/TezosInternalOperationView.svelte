...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.TezosInternalOperation>, 'prefetched'> = $props()

	const parentOperation = $derived(selection.entitySelector.$parentOperation)
		...
		href === undefined ?
			resolve(
				'/(explore)/(networks)/network/[network=networkCaip2OrNetworkSlug]/(network)/(protocol-networks)/operation-group/[operationHash=stringSegment]/(tezosOperationGroup)/operation/[contentIndex=nonNegativeInteger]/(tezosOperation)/internal/[internalIndex=nonNegativeInteger]',
				{
					...
					internalIndex: String(selection.entitySelector.internalIndex),
				}
			)
		:
			href ?? undefined
