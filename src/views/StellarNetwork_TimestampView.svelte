...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.StellarNetwork_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
...
>
	{#snippet Title()}
		<ResourceBoundary resource={stellarNetworkTimestamp}>
			{#snippet Pending()}
				{title || 'stellar network timestamp'}
			{/snippet}
	...
	{#snippet Content({ open: contentOpen })}
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					{@const latestLedger = pendingEntity.latestLedger}
			...
			>
				{#snippet Pending()}
					{@const baseFee = pendingEntity.baseFee}
			...
			>
				{#snippet Pending()}
					{@const baseReserve = pendingEntity.baseReserve}
