...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.StellarOperation>
			title?: string
	...
	>
		{#snippet children(stellarOperations)}
			...
			<EntitiesList
				...
				totalCount={stellarOperations.totalCount}
				getKey={(stellarOperation) => stellarOperation[EntityMetaKey.SelectorKey]}
				items={uniqueStellarOperations}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: stellarOperation }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.StellarOperation> })}
					...
					<StellarOperationView
						...
						prefetched={stellarOperationFields}
