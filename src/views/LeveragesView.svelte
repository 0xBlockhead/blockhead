...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.Leverage>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(leverages)}
			{@const uniqueLeverages = [...new Map(leverages.values.map((leverage) => [leverage[EntityMetaKey.SelectorKey], leverage])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={leverages.totalCount}
				getKey={(leverage) => leverage[EntityMetaKey.SelectorKey]}
				items={uniqueLeverages}
			>
				{#snippet Empty()}
					{#if emptyText != null}
