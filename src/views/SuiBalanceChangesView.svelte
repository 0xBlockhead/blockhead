...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.SuiBalanceChange>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(suiBalanceChanges)}
			{@const uniqueSuiBalanceChanges = [...new Map(suiBalanceChanges.values.map((suiBalanceChange) => [suiBalanceChange[EntityMetaKey.SelectorKey], suiBalanceChange])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.SuiBalanceChange}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={suiBalanceChanges.totalCount}
				getKey={(suiBalanceChange) => suiBalanceChange[EntityMetaKey.SelectorKey]}
				items={uniqueSuiBalanceChanges}
			>
				{#snippet Empty()}
					{#if emptyText != null}
