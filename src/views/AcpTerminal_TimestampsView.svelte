...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.AcpTerminal_Timestamp>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(acpTerminalTimestamps)}
			{@const uniqueAcpTerminalTimestamps = [...new Map(acpTerminalTimestamps.values.map((acpTerminalTimestamp) => [acpTerminalTimestamp[EntityMetaKey.SelectorKey], acpTerminalTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={acpTerminalTimestamps.totalCount}
				getKey={(acpTerminalTimestamp) => acpTerminalTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueAcpTerminalTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
