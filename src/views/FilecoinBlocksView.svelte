...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.FilecoinBlock>
			title?: string
	...
	>
		...
		{#snippet children(filecoinBlocks)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={filecoinBlocks.totalCount}
				getKey={(filecoinBlock) => filecoinBlock[EntityMetaKey.SelectorKey]}
				items={uniqueFilecoinBlocks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: filecoinBlock }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.FilecoinBlock> })}
					{@const filecoinBlockFields = { ...filecoinBlock[EntityMetaKey.Selector], ...filecoinBlock }}
					<FilecoinBlockView
						selection={select(EntityType.FilecoinBlock, filecoinBlock[EntityMetaKey.Selector], { sources: selection.sources })}
