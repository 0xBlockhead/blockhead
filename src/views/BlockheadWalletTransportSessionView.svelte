...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.BlockheadWalletTransportSession>, 'prefetched'> = $props()
	...
	const blockheadWalletTransportSession = $derived(viewSelection({
		fields: {
			transportKind: true,
			sessionKind: true,
			status: true,
		...
		href === undefined ?
			resolve(
				'/~/wallet/connection/[connectionKey=stringSegment]/transport-session/[transportSessionId=stringSegment]',
				{
					...
					transportSessionId: selection.entitySelector.transportSessionId,
				}
			)
		:
			href ?? undefined
	...
	{#snippet Content()}
		<dl data-column-item="center">
			...
			<div>
				...
				<dd>
					<ResourceBoundary
						resource={blockheadWalletTransportSession}
					>
						{#snippet children(entity)}
		...
		<dl data-column-item="center">
			...
			>
				{#snippet children(entity)}
					{@const relayProtocol = entity.relayProtocol}
			...
			>
				{#snippet children(entity)}
					{@const bridgeUrl = entity.bridgeUrl}
					{#if bridgeUrl != null}
						<div>
							<dt>bridge URL</dt>
							<dd>
			...
			>
				{#snippet children(entity)}
					{@const manifestUrl = entity.manifestUrl}
					{#if manifestUrl != null}
						<div>
							<dt>manifest URL</dt>
							<dd>
