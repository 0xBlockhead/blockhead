...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.NearValidator_Timestamp>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(nearValidatorTimestamps)}
			{@const uniqueNearValidatorTimestamps = [...new Map(nearValidatorTimestamps.values.map((nearValidatorTimestamp) => [nearValidatorTimestamp[EntityMetaKey.SelectorKey], nearValidatorTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.NearValidator_Timestamp}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={nearValidatorTimestamps.totalCount}
				getKey={(nearValidatorTimestamp) => nearValidatorTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueNearValidatorTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
