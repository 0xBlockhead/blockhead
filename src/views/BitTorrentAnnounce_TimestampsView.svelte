...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesSelection<EntityType.BitTorrentAnnounce_Timestamp>
...
>
	...
	{#snippet Item({ item: bitTorrentAnnounceTimestamp })}
		...
		<EntityView
			entityType={EntityType.BitTorrentAnnounce_Timestamp}
			entitySelector={bitTorrentAnnounceTimestamp[EntityMetaKey.Selector]}
			layout={EntityLayout.Summary}
			open={false}
			showTypeAnnotation={false}
		>
			{#snippet Title()}
				{[String((bitTorrentAnnounceTimestampFields.timestampMs) ?? '')].filter(Boolean).join(' ') || 'bit torrent announce timestamp'}
			{/snippet}

			{#snippet Value()}
				{[String((bitTorrentAnnounceTimestampFields.status) ?? '')].filter(Boolean).join(' ')}
			{/snippet}

			{#snippet HeadingAfter()}
				<span data-text="annotation">{[String((bitTorrentAnnounceTimestampFields.source) ?? '')].filter(Boolean).join(' ')}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
