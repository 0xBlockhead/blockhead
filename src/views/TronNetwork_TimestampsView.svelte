<script lang="ts">
	...
	let {
		...
		{
			entityFieldReference: EntityFieldReference<typeof schema, EntityType.TronNetwork_Timestamp>
			title?: string
			open?: boolean
			id: string
			href?: string
	...
	{#snippet body()}
		{#if open}
			...
			{@const timestamps = derive(
				parent,
				(parent): readonly Entity<typeof schema, EntityType.TronNetwork_Timestamp>[] => (
			...
			>
				...
				{#snippet Item(context)}
					<TronNetwork_TimestampView selector={context!.item[EntityMetaKey.Selector]} layout={EntityLayout.Summary} open={false} />
				{/snippet}
			</EntitiesList>
		{/if}
	{/snippet}
