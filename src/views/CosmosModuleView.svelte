...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.CosmosModule>
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.CosmosModule>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((pendingEntity.moduleName) ?? '')].filter(Boolean).join(' ') || 'Cosmos module')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={cosmosModule}>
			{#snippet Pending()}
				{[String((pendingEntity.moduleName) ?? '')].filter(Boolean).join(' ') || title || 'Cosmos module'}
			{/snippet}
