	...
	{#snippet Item({ item: solanaValidator })}
		...
		>
			...
			{#snippet HeadingAfter()}
				<span data-text="annotation">{solanaValidator.$network.name || (solanaValidatorSelector.$network.caip2 == null ? '' : `${solanaValidatorSelector.$network.caip2.namespace}:${solanaValidatorSelector.$network.caip2.reference}`) || 'Network'}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
