...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.NearReceipt>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(nearReceipts)}
			{@const uniqueNearReceipts = [...new Map(nearReceipts.values.map((nearReceipt) => [nearReceipt[EntityMetaKey.SelectorKey], nearReceipt])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.NearReceipt}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={nearReceipts.totalCount}
				getKey={(nearReceipt) => nearReceipt[EntityMetaKey.SelectorKey]}
				items={uniqueNearReceipts}
			>
				{#snippet Empty()}
					{#if emptyText != null}
