...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.HederaNetwork_Timestamp>
			title?: string
	...
	>
		...
		{#snippet children(hederaNetworkTimestamps)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={hederaNetworkTimestamps.totalCount}
				getKey={(hederaNetworkTimestamp) => hederaNetworkTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueHederaNetworkTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: hederaNetworkTimestamp }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.HederaNetwork_Timestamp> })}
					...
					<HederaNetwork_TimestampView
						selection={select(EntityType.HederaNetwork_Timestamp, hederaNetworkTimestamp[EntityMetaKey.Selector], { sources: selection.sources })}
