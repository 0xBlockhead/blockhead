...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.NftToken>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(nftTokens)}
			{@const uniqueNftTokens = [...new Map(nftTokens.values.map((nftToken) => [nftToken[EntityMetaKey.SelectorKey], nftToken])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.NftToken}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={nftTokens.totalCount}
				getKey={(nftToken) => nftToken[EntityMetaKey.SelectorKey]}
				items={uniqueNftTokens}
			>
				{#snippet Empty()}
					{#if emptyText != null}
