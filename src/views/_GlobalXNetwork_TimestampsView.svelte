...
<script lang="ts">
	...
	let {
		selection,
		title = 'Global X network observations',
		...
		id = 'GlobalXNetwork_Timestamps-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType._GlobalXNetwork_Timestamp>
			title?: string
	...
	>
		{#snippet children(globalXNetworkTimestamps)}
			...
			<EntitiesList
				...
				totalCount={globalXNetworkTimestamps.totalCount}
				getKey={(globalXNetworkTimestamp) => globalXNetworkTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueGlobalXNetworkTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No Global X network observations yet.</p>
					{/if}
				{/snippet}
