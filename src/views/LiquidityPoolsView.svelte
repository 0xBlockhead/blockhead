	...
	{#snippet Item({ item: liquidityPool })}
			...
			href={
				'caip2' in liquidityPoolSelector.$network ?
					resolve(
						...
						{
							chainId: liquidityPoolSelector.$network.caip2.reference,
		...
		>
			...
			{#snippet HeadingAfter()}
				<span data-text="annotation">{liquidityPool.$network.name || `${liquidityPool.$network.caip2.namespace}:${liquidityPool.$network.caip2.reference}` || 'Network'}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
