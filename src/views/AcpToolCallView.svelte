...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.AcpToolCall>, 'prefetched'> = $props()

	const promptTurn = $derived(selection.entitySelector.$promptTurn)
		...
		href === undefined ?
			resolve(
				'/(agents)/agents/acp/session/[sessionId=stringSegment]/(acpSession)/turn/[turnId=stringSegment]/(acpPromptTurn)/tool-call/[toolCallId=stringSegment]',
				{
					...
					toolCallId: selection.entitySelector.toolCallId,
				}
			)
		:
			href ?? undefined
