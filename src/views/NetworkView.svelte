<script lang="ts">
	...
	import { useEntity3 } from '$/collections/$queries.svelte.ts'
	import { derive } from '$/lib/svelte/RemoteResource.svelte.ts'
	import { schema } from '$/schema/index.ts'


	const networkResource = useEntity3(
		EntityType.Network,
		entityId,
		{
			$: [
	...
	import QueryResourceBoundary from '$/components/QueryResourceBoundary.svelte'
...
<QueryResourceBoundary query={derive(networkResource, (network) => network.name)}>
	{#snippet children(networkName)}
		<EntityView
		...
		>
				...
				<dl>
					...
					{#if layout === EntityLayout.SummaryDetails}
						<div>
							...
							>
								<QueryResourceBoundary query={derive(networkResource, (network) => network.blockHeight)}>
			...
			{#snippet Details({ open: _open })}
				...
				<div data-column="gap-3" data-e2e="network-carousel-groups">
					<Collapsible id={`${stringify(entityId)}:carousel-topology`} {...{ 'data-card': '' }} data-e2e="network-collapsible-topology">
					...
					<Collapsible id={`${stringify(entityId)}:carousel-economics`} {...{ 'data-card': '' }} data-e2e="network-collapsible-economics">
						...
						{#snippet children(_ctx)}
							<div class="carousel" data-scroll-container="inline layout-carousel carousel-marker-tabs" data-row="start align-start" data-e2e="network-carousel-economics">
								{#if layout === EntityLayout.SummaryDetails}<section data-e2e="network-economics-gas" data-scroll-marker-label="Gas"><div class="entity-details"><dl><div><dt>Gas price</dt><dd><QueryResourceBoundary query={derive(networkResource, (network) => network.gasPrice)}>{#snippet children(gasPrice)}<NumberValue value={gasPrice} /> wei{/snippet}</QueryResourceBoundary></dd></div></dl></div></section>{/if}
							</div>
						{/snippet}
					</Collapsible>

					<Collapsible id={`${stringify(entityId)}:carousel-execution`} {...{ 'data-card': '' }} data-e2e="network-collapsible-execution">
