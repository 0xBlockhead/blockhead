...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BlockheadWallet>
			title?: string
	...
	>
		...
		{#snippet children(blockheadWallets)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={blockheadWallets.totalCount}
				getKey={(blockheadWallet) => blockheadWallet[EntityMetaKey.SelectorKey]}
				items={uniqueBlockheadWallets}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: blockheadWallet }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.BlockheadWallet> })}
					{@const blockheadWalletFields = { ...blockheadWallet[EntityMetaKey.Selector], ...blockheadWallet }}
					<BlockheadWalletView
						selection={select(EntityType.BlockheadWallet, blockheadWallet[EntityMetaKey.Selector], { sources: selection.sources })}
