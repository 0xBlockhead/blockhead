...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesSelection<EntityType.BitTorrentDhtNode_Timestamp>
...
>
	...
	{#snippet Item({ item: bitTorrentDhtNodeTimestamp })}
		...
		<EntityView
			entityType={EntityType.BitTorrentDhtNode_Timestamp}
			entitySelector={bitTorrentDhtNodeTimestamp[EntityMetaKey.Selector]}
			layout={EntityLayout.Summary}
			open={false}
			showTypeAnnotation={false}
		>
			{#snippet Title()}
				{[String((bitTorrentDhtNodeTimestampFields.nodeId) ?? '')].filter(Boolean).join(' ') || 'bit torrent DHT node timestamp'}
			{/snippet}

			{#snippet Value()}
				{[String((bitTorrentDhtNodeTimestampFields.reachable) ?? '')].filter(Boolean).join(' ')}
			{/snippet}

			{#snippet HeadingAfter()}
				<span data-text="annotation">{[String((bitTorrentDhtNodeTimestampFields.timestampMs) ?? '')].filter(Boolean).join(' ')}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
