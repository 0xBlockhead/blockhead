...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.ZeroGServiceProvider>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((selection.entitySelector.providerId ?? prefetched.providerId) ?? '')].filter(Boolean).join(' ') || 'zero g service provider')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={zeroGServiceProvider}>
			{#snippet Pending()}
				{[String((selection.entitySelector.providerId ?? prefetched.providerId) ?? '')].filter(Boolean).join(' ') || title || 'zero g service provider'}
			{/snippet}
	...
	{#snippet Value()}
		<ResourceBoundary resource={zeroGServiceProvider}>
			{#snippet Pending()}
				{[String((prefetched.serviceKind) ?? '')].filter(Boolean).join(' ') || [String((selection.entitySelector.providerId ?? prefetched.providerId) ?? '')].filter(Boolean).join(' ') || title || 'zero g service provider'}
			{/snippet}
		...
		<dl data-column-item="center">
			...
			<div>
				...
				<dd>
					...
					>
						{#snippet Pending()}
							{@const providerId = selection.entitySelector.providerId ?? prefetched.providerId}
			...
			>
				{#snippet Pending()}
					{@const serviceKind = prefetched.serviceKind}
