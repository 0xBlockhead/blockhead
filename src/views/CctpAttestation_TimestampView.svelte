...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.CctpAttestation_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((selection.entitySelector.timestampMs ?? prefetched.timestampMs) ?? '')].filter(Boolean).join(' ') || 'CCTP attestation timestamp')
...
>
	...
	{#snippet Value()}
		<ResourceBoundary resource={cctpAttestationTimestamp}>
			{#snippet Pending()}
				{[String((prefetched.status) ?? '')].filter(Boolean).join(' ') || [String((selection.entitySelector.timestampMs ?? prefetched.timestampMs) ?? '')].filter(Boolean).join(' ') || title || 'CCTP attestation timestamp'}
			{/snippet}
	...
	{#snippet Content({ open: contentOpen })}
		...
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					{@const delayReason = prefetched.delayReason}
			...
			>
				{#snippet Pending()}
					{@const forwardState = prefetched.forwardState}
			...
			>
				{#snippet Pending()}
					{@const forwardTxHash = prefetched.forwardTxHash}
			...
			>
				{#snippet Pending()}
					{@const attestation = prefetched.attestation}
