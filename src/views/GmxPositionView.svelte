		...
		href === undefined ?
			resolve(
				'/(explore)/(networks)/network/[network=networkCaip2OrNetworkSlug]/(network)/(accounts)/account/[accountId=stringSegmentOrPolkadotAccountIdOrEvmAddressOrSolanaPubkey]/(selection)/gmx/position/[contractKey=evmTxHash]',
				{
					...
					contractKey: selection.entitySelector.contractKey,
				}
			)
		:
			href ?? undefined
				...
				<dd>
					...
					>
						{#snippet children(gmxMarket)}
							{@const gmxMarketInitial = untrack(() => gmxMarket)}
							<GmxMarketView
								selection={select(EntityType.GmxMarket, (gmxMarket ?? gmxMarketInitial)[EntityMetaKey.Selector])}
								prefetched={gmxMarket ?? gmxMarketInitial}
								layout={EntityLayout.Value}
							/>
						{/snippet}
					</ResourceBoundary>
