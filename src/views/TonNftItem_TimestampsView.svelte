...
<script lang="ts">
	...
	let {
		...
		id = 'TonNftItem_Timestamps-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.TonNftItem_Timestamp>
			title?: string
	...
	>
		...
		{#snippet children(tonNftItemTimestamps)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={tonNftItemTimestamps.totalCount}
				getKey={(tonNftItemTimestamp) => tonNftItemTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueTonNftItemTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No TON NFT item observations yet.</p>
					{/if}
				{/snippet}
