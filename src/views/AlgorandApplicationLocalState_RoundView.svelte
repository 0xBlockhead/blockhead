...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: RegisteredEntityProxyPrefetchedData<EntityType.AlgorandApplicationLocalState_Round>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const algorandApplicationLocalStateRound = $derived(selection(prefetched[EntityMetaKey.Selector] != null && layout !== EntityLayout.SummaryDetails ? {
	...
	const viewDomId = $derived('algorand-application-local-state-round-' + encodeURIComponent(stringify(selection.entitySelector ?? prefetched[EntityMetaKey.Selector])))
