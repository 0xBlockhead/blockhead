...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.ElementsAsset_Timestamp>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(elementsAssetTimestamps)}
			{@const uniqueElementsAssetTimestamps = [...new Map(elementsAssetTimestamps.values.map((elementsAssetTimestamp) => [elementsAssetTimestamp[EntityMetaKey.SelectorKey], elementsAssetTimestamp])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={elementsAssetTimestamps.totalCount}
				getKey={(elementsAssetTimestamp) => elementsAssetTimestamp[EntityMetaKey.SelectorKey]}
				items={uniqueElementsAssetTimestamps}
			>
				{#snippet Empty()}
					{#if emptyText != null}
