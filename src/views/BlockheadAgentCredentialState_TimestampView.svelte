...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyResource<typeof schema, EntityType.BlockheadAgentCredentialState_Timestamp>
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.BlockheadAgentCredentialState_Timestamp>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((selection.entitySelector.timestampMs ?? prefetched.timestampMs) ?? '')].filter(Boolean).join(' ') || 'blockhead agent credential state timestamp')
...
>
	...
	{#snippet Value()}
		<ResourceBoundary resource={blockheadAgentCredentialStateTimestamp}>
			{#snippet Pending()}
				{[String((prefetched.status) ?? '')].filter(Boolean).join(' ') || [String((selection.entitySelector.timestampMs ?? prefetched.timestampMs) ?? '')].filter(Boolean).join(' ') || title || 'blockhead agent credential state timestamp'}
			{/snippet}
