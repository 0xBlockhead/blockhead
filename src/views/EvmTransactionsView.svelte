		...
		<EntityView
			...
			href={
				resolve(
					...
					{
						network: (
							'caip2' in evmTransactionSelector.$network ?
								String(caip2StringFromValue(evmTransactionSelector.$network.caip2))
							:
								String(evmTransactionSelector.$network.slug)
						),
						transactionId: String(evmTransactionSelector.txHash),
					}
				)
			}
		>
