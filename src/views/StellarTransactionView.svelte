...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyResource<EntityType.StellarTransaction>
			prefetched?: Partial<RegisteredEntityProxyData<EntityType.StellarTransaction>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const stellarTransaction = $derived(selection({}))
	const titleFallback = $derived('stellar transaction')
	const viewDomId = $derived('stellar-transaction-' + (titleFallback.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'entity').replace(/^-|-$/g, ''))
...
>
	{#snippet Title()}
		<ResourceBoundary resource={stellarTransaction}>
			{#snippet Pending()}
