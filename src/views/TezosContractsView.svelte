...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.TezosContract>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(tezosContracts)}
			{@const uniqueTezosContracts = [...new Map(tezosContracts.values.map((tezosContract) => [tezosContract[EntityMetaKey.SelectorKey], tezosContract])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.TezosContract}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={tezosContracts.totalCount}
				getKey={(tezosContract) => tezosContract[EntityMetaKey.SelectorKey]}
				items={uniqueTezosContracts}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: tezosContract }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.TezosContract> })}
