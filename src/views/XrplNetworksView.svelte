...
<script lang="ts">
	...
	let {
		...
		id = 'XrplNetworks-list',
		...EntitiesListProps
	}: WithRest<
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.XrplNetwork>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(xrplNetworks)}
			{@const uniqueXrplNetworks = [...new Map(xrplNetworks.values.map((xrplNetwork) => [xrplNetwork[EntityMetaKey.SelectorKey], xrplNetwork])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.XrplNetwork}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={xrplNetworks.totalCount}
				getKey={(xrplNetwork) => xrplNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueXrplNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
						<p data-text="muted">{emptyText}</p>
					{:else}
						<p data-text="muted">No XRPL networks yet.</p>
					{/if}
				{/snippet}
