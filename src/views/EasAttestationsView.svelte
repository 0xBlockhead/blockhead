...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.EasAttestation>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(easAttestations)}
			{@const uniqueEasAttestations = [...new Map(easAttestations.values.map((easAttestation) => [easAttestation[EntityMetaKey.SelectorKey], easAttestation])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.EasAttestation}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={easAttestations.totalCount}
				getKey={(easAttestation) => easAttestation[EntityMetaKey.SelectorKey]}
				items={uniqueEasAttestations}
			>
				{#snippet Empty()}
					{#if emptyText != null}
