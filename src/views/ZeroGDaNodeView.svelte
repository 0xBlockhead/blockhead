...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.ZeroGDaNode>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((selection.entitySelector.nodeId ?? prefetched.nodeId) ?? '')].filter(Boolean).join(' ') || 'zero g da node')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={zeroGDaNode}>
			{#snippet Pending()}
				{[String((selection.entitySelector.nodeId ?? prefetched.nodeId) ?? '')].filter(Boolean).join(' ') || title || 'zero g da node'}
			{/snippet}
