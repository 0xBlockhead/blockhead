...
<script lang="ts">
	...
	let {
	...
	}: WithRest<
		{
			...
			prefetched?: Partial<EntityProxyData<typeof schema, EntityType.QuilibriumProver>>
			title?: string
			href?: string
			layout?: EntityLayout
			open?: boolean
	...
	const titleFallback = $derived([String((selection.entitySelector.proverPeerId ?? prefetched.proverPeerId) ?? '')].filter(Boolean).join(' ') || 'quilibrium prover')
...
>
	{#snippet Title()}
		<ResourceBoundary resource={quilibriumProver}>
			{#snippet Pending()}
				{[String((selection.entitySelector.proverPeerId ?? prefetched.proverPeerId) ?? '')].filter(Boolean).join(' ') || title || 'quilibrium prover'}
			{/snippet}
	...
	{#snippet HeadingAfter()}
		<ResourceBoundary resource={quilibriumProver}>
			{#snippet Pending()}
				{@const version0 = prefetched.version}
		...
		<dl data-column-item="center">
			...
			<div>
				...
				<dd>
					...
					>
						{#snippet Pending()}
							{@const proverPeerId = selection.entitySelector.proverPeerId ?? prefetched.proverPeerId}
