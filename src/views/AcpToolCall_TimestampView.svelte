...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: EntitySelectionViewProps<EntityType.AcpToolCall_Timestamp> = $props()
	...
	const acpToolCallTimestamp = $derived(viewSelection({
		fields: {
			status: true,
			latencyMs: true,
		},
	}))
	const titleFallback = $derived(String(pendingEntity.timestampMs ?? '') || 'ACP tool call timestamp')
