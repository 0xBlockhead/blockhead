...
<script lang="ts">
	...
	const titleFallback = $derived([String((({ ...selection.entitySelector, ...prefetched }).protocolName) ?? '')].filter(Boolean).join(' ') || 'X')
...
>
	{#snippet Title()}
		{#if layout === EntityLayout.Summary || layout === EntityLayout.SummaryInline}
			{[String((({ ...selection.entitySelector, ...prefetched }).protocolName) ?? '')].filter(Boolean).join(' ') || title || 'X'}
		{:else}
	...
	{#snippet TypeAnnotationTooltip()}
		<p>
			X profiles and posts surfaced through configured public HTTP sources.
		</p>
	{/snippet}
