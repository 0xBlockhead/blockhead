...
<script lang="ts">
	...
	let {
		...
		...EntityViewProps
	}: Omit<EntitySelectionViewProps<EntityType.NearAccessKey_Timestamp>, 'prefetched'> = $props()

	const accessKey = $derived(selection.entitySelector.$accessKey)
	...
	href={
		href === undefined ?
			resolve(
				...
				{
					network: (
						accessKey.$account.$network.caip2 !== undefined ?
