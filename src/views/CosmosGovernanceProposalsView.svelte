	...
	{#snippet Item({ item: cosmosGovernanceProposal })}
		...
		>
			...
			{#snippet HeadingAfter()}
				<span data-text="annotation">{cosmosGovernanceProposal.$network.name || `${cosmosGovernanceProposal.$network.caip2.namespace}:${cosmosGovernanceProposal.$network.caip2.reference}` || 'Network'}</span>
			{/snippet}
		</EntityView>
	{/snippet}
</EntitiesList>
