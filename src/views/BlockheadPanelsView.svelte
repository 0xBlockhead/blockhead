...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BlockheadPanel>
			title?: string
	...
	>
		...
		{#snippet children(blockheadPanels)}
			...
			>
				...
				{#snippet Item({ item: blockheadPanel }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.BlockheadPanel> })}
					{@const blockheadPanelFields = { ...blockheadPanel[EntityMetaKey.Selector], ...blockheadPanel }}
					<BlockheadPanelView
						selection={select(EntityType.BlockheadPanel, blockheadPanel[EntityMetaKey.Selector], { sources: selection.sources })}
