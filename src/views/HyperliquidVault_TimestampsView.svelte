...
<script lang="ts">
	...
	let {
		...
		{
			selection: RegisteredEntityProxyEntitiesResource<EntityType.HyperliquidVault_Timestamp>
			title?: string
...
<EntitiesList
	...
	getResourceItems={(hyperliquidVaultTimestamps) => [...new Map(hyperliquidVaultTimestamps.values.map((hyperliquidVaultTimestamp) => [hyperliquidVaultTimestamp[EntityMetaKey.SelectorKey], hyperliquidVaultTimestamp])).values()]}
...
>
	...
	{#snippet Item({ item: hyperliquidVaultTimestamp })}
		...
		{@const selection = select(EntityType.HyperliquidVault_Timestamp, hyperliquidVaultTimestamp[EntityMetaKey.Selector], { sources: collectionSelection.sources })}
