<script lang="ts">
	...
	let {
		...
		{
			entityId: EntityId<typeof schema, EntityType.Network_BeaconFinality_Timestamp>
			href?: string
			layout?: EntityLayout
			open?: boolean
		},
	...
	const networkBeaconFinalityTimestamp = useEntity(
		EntityType.Network_BeaconFinality_Timestamp,
		entityId,
		{
			$: [
				Source.Beacon_Rest,
...
>
	...
	{#snippet Content({ title: _title, href: _href })}
		...
		>
			{#snippet children(loadedNetworkBeaconFinalityTimestamp)}
						...
						<dd data-row="wrap align-start gap-2">
							<BeaconEpochView
								entityId={{
									...
									epoch: loadedNetworkBeaconFinalityTimestamp.currentJustifiedCheckpointEpoch,
								}}
							...
							<TruncatedValue
								...
								value={loadedNetworkBeaconFinalityTimestamp.currentJustifiedCheckpointRoot}
							/>
						</dd>
					</div>
					<div>
						...
						<dd data-row="wrap align-start gap-2">
							<BeaconEpochView
								entityId={{
									$network: { chainId: entityId.$network.chainId },
									epoch: loadedNetworkBeaconFinalityTimestamp.finalizedCheckpointEpoch,
								}}
								layout={EntityLayout.Title}
								open={false}
							/>
							<TruncatedValue
								format={TruncatedValueFormat.Abbr}
								value={loadedNetworkBeaconFinalityTimestamp.finalizedCheckpointRoot}
							/>
						</dd>
					</div>
					<div>
						...
						<dd data-row="wrap align-start gap-2">
							<BeaconEpochView
								entityId={{
									$network: { chainId: entityId.$network.chainId },
									epoch: loadedNetworkBeaconFinalityTimestamp.previousJustifiedCheckpointEpoch,
								}}
								layout={EntityLayout.Title}
								open={false}
							/>
							<TruncatedValue
								format={TruncatedValueFormat.Abbr}
								value={loadedNetworkBeaconFinalityTimestamp.previousJustifiedCheckpointRoot}
							/>
						</dd>
					</div>
				</dl>
