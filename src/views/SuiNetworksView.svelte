...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.SuiNetwork>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(suiNetworks)}
			{@const uniqueSuiNetworks = [...new Map(suiNetworks.values.map((suiNetwork) => [suiNetwork[EntityMetaKey.SelectorKey], suiNetwork])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.SuiNetwork}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={suiNetworks.totalCount}
				getKey={(suiNetwork) => suiNetwork[EntityMetaKey.SelectorKey]}
				items={uniqueSuiNetworks}
			>
				{#snippet Empty()}
					{#if emptyText != null}
