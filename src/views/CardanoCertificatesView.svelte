...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.CardanoCertificate>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(cardanoCertificates)}
			{@const uniqueCardanoCertificates = [...new Map(cardanoCertificates.values.map((cardanoCertificate) => [cardanoCertificate[EntityMetaKey.SelectorKey], cardanoCertificate])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={cardanoCertificates.totalCount}
				getKey={(cardanoCertificate) => cardanoCertificate[EntityMetaKey.SelectorKey]}
				items={uniqueCardanoCertificates}
			>
				{#snippet Empty()}
					{#if emptyText != null}
