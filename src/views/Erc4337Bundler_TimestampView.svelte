...
<script lang="ts">
	...
	const titleFallback = $derived([String((({ ...selection.entitySelector, ...prefetched }).timestampMs) ?? '')].filter(Boolean).join(' ') || 'ERC-4337 bundler timestamp')
...
<EntityView
	...
	href={
		href ?? resolve('/(explore)/(networks)/network/[caip2=eip155NetworkCaip2]/(network)/erc-4337/bundler/[address=evmAddress]/observations/[timestampMs=nonNegativeInteger]/[source]', {
			caip2: `${String(({ ...selection.entitySelector, ...prefetched }).$bundler.$network.caip2.namespace)}:${String(({ ...selection.entitySelector, ...prefetched }).$bundler.$network.caip2.reference)}`,
			address: String(({ ...selection.entitySelector, ...prefetched }).$bundler.address),
					...
					<Erc4337BundlerView
						...
						href={
							resolve('/(explore)/(networks)/network/[caip2=eip155NetworkCaip2]/(network)/erc-4337/bundler/[address=evmAddress]', {
								caip2: `${String(selection.entitySelector.$bundler.caip2.namespace)}:${String(selection.entitySelector.$bundler.caip2.reference)}`,
