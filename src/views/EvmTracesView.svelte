...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.EvmTrace>
			title?: string
...
{#if open}
	...
	>
		...
		{#snippet children(evmTraces)}
			{@const uniqueEvmTraces = [...new Map(evmTraces.values.map((evmTrace) => [evmTrace[EntityMetaKey.SelectorKey], evmTrace])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				entityType={EntityType.EvmTrace}
				{id}
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={evmTraces.totalCount}
				getKey={(evmTrace) => evmTrace[EntityMetaKey.SelectorKey]}
				items={uniqueEvmTraces}
			>
				{#snippet Empty()}
					{#if emptyText != null}
