...
<script lang="ts">
	...
	const titleFallback = $derived([String((selection.entitySelector.id ?? prefetched.id) ?? '')].filter(Boolean).join(' ') || 'liquidity pool')
...
<EntityView
	...
	href={
		href ?? (pendingEntity.$network !== undefined && pendingEntity.$network.caip2 !== undefined && pendingEntity.$network.caip2.reference !== undefined && pendingEntity.id !== undefined ? resolve('/(assets)/pool/[chainId=eip155ChainId]/[poolId]', {
...
>
	...
	{#snippet Content({ open: contentOpen })}
		...
		<dl data-column-item="center">
			<ResourceBoundary
				resource={
					selection[EntityProxyField]<EntityType.EvmContract, false>('$baseToken', {
						sources: [
							Source.Dexscreener_OpenApi,
						],
					})
			...
			<ResourceBoundary
				resource={
					selection[EntityProxyField]<EntityType.EvmContract, false>('$quoteToken', {
						sources: [
							Source.Dexscreener_OpenApi,
						],
					})
		...
		<dl data-column-item="center">
			...
			>
				{#snippet Pending()}
					{@const tickSpacing = prefetched.tickSpacing}
			...
			>
				{#snippet Pending()}
					{@const v4PoolId = prefetched.v4PoolId}
	...
	{#snippet Details({ open: detailsOpen })}
		{#if detailsOpen}
			<LiquidityPool_TimestampsView
				selection={
						selection[EntityProxyField]<EntityType.LiquidityPool_Timestamp>('$$timestamps', {
							sources: [
								Source.Dexscreener_OpenApi,
							],
						})
			...
			<LeveragesView
				selection={selection[EntityProxyField]<EntityType.Leverage>('$$leverages')}
				title='Leverage positions'
				emptyText='No leverage positions yet.'
				id='LeveragesView-$$leverages'
			/>
		{/if}
	{/snippet}
</EntityView>
