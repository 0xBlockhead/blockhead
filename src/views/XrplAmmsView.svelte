...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.XrplAmm>
			title?: string
	...
	>
		...
		{#snippet children(xrplAmms)}
			...
			<EntitiesList
				...
				{showTypeAnnotation}
				TypeAnnotationTooltip={typeAnnotationParagraphs.length > 0 ? TypeAnnotationParagraphs : undefined}
				totalCount={xrplAmms.totalCount}
				getKey={(xrplAmm) => xrplAmm[EntityMetaKey.SelectorKey]}
				items={uniqueXrplAmms}
			>
				{#snippet Empty()}
					{#if emptyText != null}
				...
				{#snippet Item({ item: xrplAmm }: { item: SubscribeEntityReferenceResult<typeof schema, EntityType.XrplAmm> })}
					{@const xrplAmmFields = { ...xrplAmm[EntityMetaKey.Selector], ...xrplAmm }}
					<XrplAmmView
						selection={select(EntityType.XrplAmm, xrplAmm[EntityMetaKey.Selector], { sources: selection.sources })}
