...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.GitTree>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(gitTrees)}
			{@const uniqueGitTrees = [...new Map(gitTrees.values.map((gitTree) => [gitTree[EntityMetaKey.SelectorKey], gitTree])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={gitTrees.totalCount}
				getKey={(gitTree) => gitTree[EntityMetaKey.SelectorKey]}
				items={uniqueGitTrees}
			>
				{#snippet Empty()}
					{#if emptyText != null}
