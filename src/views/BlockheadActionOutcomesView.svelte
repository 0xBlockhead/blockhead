...
<script lang="ts">
	...
	let {
		...
		{
			selection: EntityProxyEntitiesResource<typeof schema, EntityType.BlockheadActionOutcome>
			title?: string
...
{#if open}
	<ResourceBoundary
		...
		{placeholderText}
	>
		{#snippet children(blockheadActionOutcomes)}
			{@const uniqueBlockheadActionOutcomes = [...new Map(blockheadActionOutcomes.values.map((blockheadActionOutcome) => [blockheadActionOutcome[EntityMetaKey.SelectorKey], blockheadActionOutcome])).values()]}
			<EntitiesList
				{...EntitiesListProps}
				...
				totalCount={blockheadActionOutcomes.totalCount}
				getKey={(blockheadActionOutcome) => blockheadActionOutcome[EntityMetaKey.SelectorKey]}
				items={uniqueBlockheadActionOutcomes}
			>
				{#snippet Empty()}
					{#if emptyText != null}
