...
describe('Farcaster REST binding authority', () => {
	it('routes client and web APIs through their declared proxy endpoints', async () => {
		...
		expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
			`/api-proxy/${encodeURIComponent(farcasterBinding.proxyId)}/0/https%3A%2F%2Fapi.farcaster.xyz%2Fv2%2Fall-channels%3Flimit%3D100`,
			`/api-proxy/${encodeURIComponent(farcasterBinding.proxyId)}/1/https%3A%2F%2Ffarcaster.xyz%2F~api%2Fv2%2Fuser-thread-casts%3Fusername%3Dalice`,
		])
	})
})
