...
export const getCoinOhlc = async ({
	...
	const lookbackDays = marketOhlcDayLookbackValues.find((dayCount) => dayCount === lookbackDayCount)
	if (lookbackDays == null) return []

	const searchParams = new URLSearchParams()
	searchParams.set('vs_currency', vsCurrency)
	searchParams.set('days', String(lookbackDays))
...
export const collectDerivativeMarketEntitySelectors = async ({
	...
	return (await Promise.all(
		Object.values(MarketVenueId)
			.flatMap((venueId) => {
				const exchangeId = coingeckoDerivativesExchangeIdByMarketVenueId[venueId]
				...
				return (exchange?.tickers ?? []).flatMap((ticker) => {
					...
					if (
						...
						&& (
							...
							|| marketId.$base.assetKey !== catalogCoinId
						)
					)
						return []
