...
const {
	...
	searchPosts,
} = await import('$/sources/AtprotoBskySocial/Rest/queries.ts')
...
it('uses only the registered Bsky Social proxy binding for all seven operations', async () => {
	...
	for (const [binding] of sourceGetJson.mock.calls) {
		expect(binding.source).toBe('Atproto_BskySocial_Xrpc')
...
it('preserves repeated AT URIs and reserved parameters without a direct bypass', async () => {
