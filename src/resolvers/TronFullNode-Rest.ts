...
const tronFullNodeRestUrl = 'http://127.0.0.1:8090'
...
const assertTronMainnet = (network: { namespace: string; reference: string }) => {
	if (network.namespace !== NetworkNamespace.Tron || network.reference !== '0x2b6653dc') {
		throw new Error(`TronFullNode_Rest: unsupported network ${network.namespace}:${network.reference}`)
	}
}
			...
			resolve: async (entityId) => {
				...
				return blockFields(
					...
					await getBlockByNumber({
						restBaseUrl: tronFullNodeRestUrl,
