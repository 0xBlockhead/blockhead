...
const { default: suiGraphqlResolvers } = await import('$/resolvers/Sui-Graphql.ts')

const resolverFor = (fieldName: '$$balances' | '$$transactions') => {
	const resolver = suiGraphqlResolvers.resolvers.find((candidate) => (
		fieldName in candidate.projections
	))
	if (resolver == null)
		throw new Error(`Sui GraphQL spec missing SuiAccount.${fieldName} resolver`)
...
describe('Sui GraphQL public-account resolver', () => {
	...
	it('materializes normalized source-provenanced balances with lossless amounts', async () => {
		...
		expect(resolver.projections.$$balances.select(snapshot, account, context)).toEqual([
			{
		...
		expect(resolver.projections.$$balances.continuation(snapshot, account, context)).toEqual({
