	...
	entityResolvers: [
		defineEntityResolver({
			...
			resolve: async (entityId, context) => {
				const { queryAccount } = await import('$/sources/Hey/Graphql/queries.ts')
		...
		defineEntityResolver({
			...
			resolve: async (entityId, context) => {
				const { queryPost } = await import('$/sources/Hey/Graphql/queries.ts')
				const publicEnv = sourcePublicEnv(context, Source.Hey_Graphql)
	...
	entityFieldResolvers: [
		defineEntityFieldResolver({
			...
			resolve: async (_entityId, context) => {
				const { queryLatestPosts } = await import('$/sources/Hey/Graphql/queries.ts')
				const publicEnv = sourcePublicEnv(context, Source.Hey_Graphql)
		...
		defineEntityFieldResolver({
			...
			fieldName: '$$comments',
			resolve: async (entityId, context) => {
				const { queryPostComments } = await import('$/sources/Hey/Graphql/queries.ts')
				const publicEnv = sourcePublicEnv(context, Source.Hey_Graphql)
				const limit = resolverLoadSubsetRowLimit(context)
				const pageSize: 'TEN' | 'FIFTY' = limit > 10 ? 'FIFTY' : 'TEN'
				return (
		...
		defineEntityFieldResolver({
			...
			fieldName: '$$posts',
			resolve: async (entityId, context) => {
				const { queryPostsByAuthor } = await import('$/sources/Hey/Graphql/queries.ts')
				const publicEnv = sourcePublicEnv(context, Source.Hey_Graphql)
				const limit = resolverLoadSubsetRowLimit(context)
				const pageSize: 'TEN' | 'FIFTY' = limit > 10 ? 'FIFTY' : 'TEN'
				return (
