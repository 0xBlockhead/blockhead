	...
	resolvers: [
			...
			resolve: {
				FromToUrl: {
					resolve: async (entitySelector) => {
						...
						if (parentChainId == null) throw new Error('Chainlist_Rest: network bridge not in rpcs.json')
						if (
		...
		defineResolver(Source.Chainlist_Rest, {
			...
			Evm: {
				...
				shortName: ({ chain }) => (
					chain.shortName == null || String(chain.shortName).length === 0 ?
						undefined
					:
						String(chain.shortName)
				),
				registryStatus: ({ chain }) => (
					chain.status == null || String(chain.status).length === 0 ?
						undefined
					:
						String(chain.status)
				),
				...
				Scope: {
					resolve: async (_entitySelector) => {
						...
						return (await fetchRpcsJson())
							.flatMap((chain) => (
							[{ [EntityMetaKey.Selector]: evmNetworkSelectorFromChainId(chain.chainId) }]
							))
					},
				}
			},
