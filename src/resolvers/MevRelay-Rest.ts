...
export default {
	...
	resolvers: [
		...
		defineResolver(Source.MevRelay_Rest, {
			...
			resolve: {
				[MevRelaySelector.EvmNetworkHost]: async (entitySelector) => [
					{
						[EntityMetaKey.Selector]: {
		...
		defineResolver(Source.MevRelay_Rest, {
			...
			resolve: {
				[MevRelay_TimestampSelector.RelayTimestampMsSource]: async ({ $relay, source }, context) => {
		...
		defineResolver(Source.MevRelay_Rest, {
			...
			resolve: {
				[MevBuilderSelector.EvmNetworkBuilderPubkey]: async (entitySelector) => [
					{
						[EntityMetaKey.Selector]: {
		...
		defineResolver(Source.MevRelay_Rest, {
			...
			resolve: {
				[MevBuilder_TimestampSelector.BuilderTimestampMsSource]: async ({ $builder, source }) => {
					if (source !== Source.MevRelay_Rest)
		...
		defineResolver(Source.MevRelay_Rest, {
			entityType: EntityType.MevBuilder,
			resolve: {
				[MevBuilderSelector.EvmNetworkBuilderPubkey]: async (entitySelector, context) => {
					const { mevRelayHosts } = await import('$/constants/MevRelayHosts.ts')
					const { getProposerPayloadDeliveredForRelayHost } = await import('$/sources/MevRelay/Rest/queries.ts')
