			...
			resolve: {
				NetworkHeight: {
					resolve: async ({ $network, height }) => {
						assertDogecoinMainnet($network)
						const { getBlockHash } = await import('$/sources/DogecoinCore/JsonRpc/queries.ts')
			...
			resolveLive: {
				utxoHead: {
					...
					start: ({
						...
						assertDogecoinMainnet(parentEntitySelector)
						...
						const poll = async () => {
							try {
								if (signal.aborted)
									return
								const { getBlockCount } = await import('$/sources/DogecoinCore/JsonRpc/queries.ts')
								const tipHeight = await getBlockCount()
								if (signal.aborted)
									return
								...
								console.error('DogecoinCore_JsonRpc live UTXO head failed', error)
							}
							if (signal.aborted)
								return
					...
					start: ({
						...
						const poll = async () => {
							try {
								...
								console.error('DogecoinCore_JsonRpc live network head failed', error)
							}
							if (signal.aborted)
								return
							timeout = setTimeout(() => { void poll() }, 15_000)
