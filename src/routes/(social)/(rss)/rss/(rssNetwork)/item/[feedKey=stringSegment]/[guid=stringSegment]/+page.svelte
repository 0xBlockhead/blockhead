...
<script lang="ts">
	...
	const pageEntityTitle = $derived((data.title ?? (pageSelection.entity == null ? [String((pageSelection.entitySelector.title) ?? ''), String((pageSelection.entitySelector.guid) ?? '')].filter(Boolean).join(' ') || 'RSS item' : [String((({ ...pageSelection.entitySelector, ...pageSelection.entity }).title) ?? ''), String((({ ...pageSelection.entitySelector, ...pageSelection.entity }).guid) ?? '')].filter(Boolean).join(' ') || 'RSS item')))
