...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.BlockheadZcashNoteState, data.selector, {
		sources: [
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'blockhead zcash note state'} • blockhead zcash note state • Blockhead</title>
	{/if}
</svelte:head>
