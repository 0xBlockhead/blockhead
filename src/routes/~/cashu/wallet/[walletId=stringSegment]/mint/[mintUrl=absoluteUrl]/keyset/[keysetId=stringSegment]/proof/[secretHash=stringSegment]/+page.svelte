...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.BlockheadCashuProof, data.selector))
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'blockhead Cashu proof'} • blockhead Cashu proof • Blockhead</title>
	{/if}
</svelte:head>
