...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.BlockheadMoneroTransferState_Timestamp, {
		$transferState: data.selector,
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'blockhead monero transfer state timestamp'} • blockhead monero transfer state timestamp • Blockhead</title>
	{/if}
</svelte:head>
