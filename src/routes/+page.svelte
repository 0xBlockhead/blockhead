<script module lang="ts">
	...
	const schema = [
		{
			...
			fields: [
				...
				},
				{
					...
					cardinality: EntityFieldCardinality.Many,
					defaultSources: [Source.ChainList],
				},
			],
		},
	...
	const entityResolvers = [
		...
		defineEntityResolver({
			...
			source: Source.Eips,
			resolve: async ($id) => {
				const { getText } = await import('$/lib/fetch.ts')
	...
	const entityFieldResolvers = [
		defineEntityFieldResolver({
			...
			source: Source.Eips,
			resolve: async () => {
				const { getJson } = await import('$/lib/fetch.ts')
				const { githubRestHeaders } = await import('$/sources/Github/Rest/constants.ts')
		...
		defineEntityFieldResolver({
			...
			resolve: async ($id) => {
				...
				for (const txOrHashInBlock of transactions) {
					...
					if (typeof txOrHashInBlock === 'object' && txOrHashInBlock != null) {
					...
					$$proposalsEips: toArray(
						queryBuilder.from({ $proposal: entityFieldCollections[BlockheadEntityType._Global]['$$proposalsEips'] })
							...
							.select(({ proposal }) => ({
								...
								title: toArray(
									queryBuilder.from({ title: entityFieldCollections[BlockheadEntityType.Proposal]['title'] })
										.where(({ title }) => (
											eq(
								...
								body: toArray(
									queryBuilder.from({ body: entityFieldCollections[BlockheadEntityType.Proposal]['body'] })
										.where(({ body }) => (
											eq(
		...
		(queryBuilder) => (
			queryBuilder
				...
				.leftJoin(
					{
						title: queryBuilder
							.from({ title: entityFieldCollections[BlockheadEntityType.Proposal]['title'] })
							.where(({ title }) => (
								eq(
									title[EntityMetaKey.Source],
									Source.Eips,
				...
				.leftJoin(
					{
						body: queryBuilder
							.from({ body: entityFieldCollections[BlockheadEntityType.Proposal]['body'] })
							.where(({ body }) => (
								eq(
									body[EntityMetaKey.Source],
									Source.Eips,
				...
				.select(({ proposal, title, body }) => ({
					...
					license: proposal.license,
					supersededBy: proposal.supersededBy,
					title: title?.[EntityMetaKey.Value] ?? proposal.title,
					body: body?.[EntityMetaKey.Value],
				}))
		),
	)

	const liveQueryDataRows = (data: unknown) => (data == null ? [] : Array.isArray(data) ? data : [data])
