...
export const load: PageLoad = async ({ params, parent }) => {
	...
	if (!((projectionNetwork.executionModels !== undefined && projectionNetwork.executionModels.some((value: string | number | boolean | null) => value === 'Evm')) && matchUserOperationHash(params.userOperationHash))) error(404, 'Route mapping not applicable')

	const evmUserOperationEvmNetworkHashSelector = parseEntitySelector(
		schema,
	...
	if (evmUserOperationEvmNetworkHashSelector instanceof arktype.errors) error(404, 'Invalid EvmUserOperation selector')
