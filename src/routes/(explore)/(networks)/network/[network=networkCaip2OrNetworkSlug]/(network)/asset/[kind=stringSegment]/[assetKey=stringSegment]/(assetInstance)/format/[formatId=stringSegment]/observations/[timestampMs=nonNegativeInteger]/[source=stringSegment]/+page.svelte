...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.AssetFormatSupport_Timestamp, {
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'asset format support timestamp'} • asset format support timestamp • Blockhead</title>
	{/if}
</svelte:head>
