...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.CelestiaNamespace_Timestamp, {
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'celestia namespace timestamp'} • celestia namespace timestamp • Blockhead</title>
	{/if}
</svelte:head>
