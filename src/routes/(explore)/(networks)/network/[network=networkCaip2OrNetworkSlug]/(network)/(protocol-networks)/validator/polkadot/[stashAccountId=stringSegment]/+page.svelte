...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.PolkadotValidator, data.selector))
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'Polkadot validator'} • Polkadot validator • Blockhead</title>
	{/if}
</svelte:head>
