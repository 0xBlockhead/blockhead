...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.AptosAccountResource, data.selector))
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'aptos account resource'} • aptos account resource • Blockhead</title>
	{/if}
</svelte:head>
