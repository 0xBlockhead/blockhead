...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.SolanaTokenMint, data.selector))
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'solana token mint'} • solana token mint • Blockhead</title>
	{/if}
</svelte:head>
