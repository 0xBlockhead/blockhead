...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.BlockheadLightningPayment_Timestamp, {
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'local LND payment observation'} • local LND payment observation • Blockhead</title>
	{/if}
</svelte:head>
