...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.FilecoinDeal_Timestamp, {
		$deal: data.selector,
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'filecoin deal timestamp'} • filecoin deal timestamp • Blockhead</title>
	{/if}
</svelte:head>
