...
<script lang="ts">
	...
	const pageSelection = $derived(data?.selector == null ? undefined : select(EntityType.AlgorandApplication, data.selector))
...
<svelte:head>
	...
	{:else}
		<title>{data?.title ?? 'algorand application'} • algorand application • Blockhead</title>
	{/if}
</svelte:head>
