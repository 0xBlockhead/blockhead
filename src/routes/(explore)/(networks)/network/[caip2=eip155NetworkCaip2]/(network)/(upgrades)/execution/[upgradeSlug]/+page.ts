...
import { networkExecutionUpgradeByChainIdAndRouteSegment } from '$/constants/EthereumNetworkUpgrades.ts'
...
export const load: PageLoad = ({ params }) => {
	const ethereumExecutionUpgradeSelector = parseEntitySelector(
		...
		EthereumExecutionUpgradeSchema,
		{
			...
			upgradeId: networkExecutionUpgradeByChainIdAndRouteSegment[String(caip2SelectorValueFromString(decodeURIComponent(params.caip2)).reference) + ':' + String(params.upgradeSlug)].upgradeId,
		}
	)
