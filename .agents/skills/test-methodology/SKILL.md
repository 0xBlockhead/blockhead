...
- For a source-specific matrix sharing canonical selector fixtures, validate fixture closure once per unique typed identity while retaining execution per source. Report a pre-execution fixture gap as owner debt; do not multiply it into resolver failures or hide it with synthesized selectors, placeholders, skips, or unsupported classifications.
