...
- A next-work plan does not launch work. Given the plan and a later "go", an executor can identify the simultaneous ready set, every deferred action's exact prerequisites, and the narrow scope of resource serialization without reconstructing the reasoning. Use the Aug 26 Tauri case: independent platform diagnosis belongs explicitly alongside migration research, while only dependent migration actions wait for its findings. Reject plans that merely say other work "remains runnable", imply a whole-batch barrier, or turn this planning skill into an execution workflow.
